...
CMasternode::CMasternode(const CMasternodeBroadcast& mnb) :
    masternode_info_t{ mnb.nActiveState, mnb.nProtocolVersion, mnb.sigTime,
                       mnb.vin.prevout, mnb.addr, mnb.pubKeyCollateralAddress, mnb.pubKeyMasternode, mnb.sigTime},
...
bool CMasternode::UpdateFromNewBroadcast(CMasternodeBroadcast& mnb, CConnman& connman)
{
    ...
    nTimeLastChecked = 0;
    int nDos = 0;
    if (mnb.lastPing == CMasternodePing() || (mnb.lastPing != CMasternodePing() && (fMasterNodesReleased ? mnb.lastPing.CheckAndUpdateMnRel(this, true, nDos, connman) : mnb.lastPing.CheckAndUpdate(this, true, nDos, connman)))) {
        lastPing = mnb.lastPing;
...
arith_uint256 CMasternode::CalculateScore(const uint256& blockHash)
{
    ...
    if (fMasterNodesReleased) {
        ...
        aux = ArithToUint256(UintToArith256(pubKeyMasternode.GetHash()));
    }
...
CMasternode::CollateralStatus CMasternode::CheckCollateral(const COutPoint& outpoint, int& nHeightRet)
{
    ...
    if(coins.vout[outpoint.n].nValue != MNODE_REQ_BALANCE * COIN) {
...
// SPORK_14_MNODES_RELEASE_ENABLED active
void CMasternode::CheckMnRel(bool fForce)
{
    LOCK(cs);
    ...
    LogPrint("masternode", "CMasternode::CheckMnRel -- Masternode %s is in %s state\n", vin.prevout.ToStringShort(), GetStateString());
    ...
    if (!fUnitTest) {
        ...
        if (err == COLLATERAL_UTXO_NOT_FOUND) {
            ...
            LogPrint("masternode", "CMasternode::CheckMnRel -- Failed to find Masternode UTXO, masternode=%s\n", vin.prevout.ToStringShort());
        ...
        if (err == COLLATERAL_INVALID_AMOUNT) {
            nActiveState = MASTERNODE_OUTPOINT_SPENT;
            LogPrint("masternode", "CMasternode::CheckMnRel -- Masternode UTXO should have %d FUTURO, masternode=%s\n", MNODE_REQ_BALANCE, vin.prevout.ToStringShort());
    ...
    if (IsPoSeBanned()) {
        ...
        LogPrintf("CMasternode::CheckMnRel -- Masternode %s is unbanned and back in list now\n", vin.prevout.ToStringShort());
        DecreasePoSeBanScore();
    } else if(nPoSeBanScore >= MASTERNODE_POSE_BAN_MAX_SCORE) {
        nActiveState = MASTERNODE_POSE_BAN;
        // ban for the whole payment cycle
        nPoSeBanHeight = nHeight + mnodeman.size();
        LogPrintf("CMasternode::CheckMnRel -- Masternode %s is banned till block %d now\n", vin.prevout.ToStringShort(), nPoSeBanHeight);
        return;
    }
    ...
    if(fRequireUpdate) {
        ...
        if(nActiveStatePrev != nActiveState) {
            LogPrint("masternode", "CMasternode::CheckMnRel -- Masternode %s is in %s state now\n", vin.prevout.ToStringShort(), GetStateString());
        }
        return;
    }
    ...
    if(fWaitForPing && !fOurMasternode) {
        ...
        if(IsExpired() || IsNewStartRequired()) {
            LogPrint("masternode", "CMasternode::CheckMnRel -- Masternode %s is in %s state, waiting for ping\n", vin.prevout.ToStringShort(), GetStateString());
            return;
        }
    }
    ...
    if(!fWaitForPing || fOurMasternode) {
        ...
        LogPrint("masternode", "CMasternode::CheckMnRel -- outpoint=%s, GetAdjustedTime()=%d\n",
...
bool CMasternode::IsInputAssociatedWithPubkey()
{
    ...
    if(GetTransaction(vin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        BOOST_FOREACH(CTxOut out, tx.vout)
            if(out.nValue == MNODE_REQ_BALANCE * COIN && out.scriptPubKey == payee) return true;
    }
...
void CMasternode::UpdateLastPaid(const CBlockIndex *pindex, int nMaxBlocksToScanBack)
{
    ...
    if (fMasterNodesReleased) {
    	...
    	mnpayee = GetScriptForDestination(payee.Get());
    }
...
bool CMasternodeBroadcast::Create(std::string strService, std::string strKeyMasternode, std::string strPayee, std::string& strErrorRet, CMasternodeBroadcast &mnbRet, bool fOffline)
{
...
bool CMasternodeBroadcast::Create(const COutPoint& outpoint, const CService& service, const CKey& keyCollateralAddressNew, const CPubKey& pubKeyCollateralAddressNew, const CKey& keyMasternodeNew, const CPubKey& pubKeyMasternodeNew, std::string &strErrorRet, CMasternodeBroadcast &mnbRet)
{
    ...
    if (!mnp.SignMnRel(keyMasternodeNew, pubKeyMasternodeNew))
    ...
    mnbRet.lastPing = mnp;
    if (!mnbRet.SignMnRel(keyCollateralAddressNew))
        return Log(strprintf("Failed to sign broadcast, masternode=%s", outpoint.ToStringShort()));

    return true;
}

bool CMasternodeBroadcast::Create(const CService& service, const CKey& keyMasternodeNew, const CPubKey& pubKeyMasternodeNew, const CBitcoinAddress& payeeNew, std::string &strErrorRet, CMasternodeBroadcast &mnbRet)
{
    // wait for reindex and/or import to finish
    if (fImporting || fReindex) return false;
...
// SPORK_14_MNODES_RELEASE_ENABLED active
bool CMasternodeBroadcast::SimpleCheckMnRel(int& nDos)
{
    nDos = 0;

    // make sure addr is valid
    if(!IsValidNetAddr()) {
        LogPrintf("CMasternodeBroadcast::SimpleCheckMnRel -- Invalid addr, rejected: masternode=%s  addr=%s\n",
    ...
    if (sigTime > GetAdjustedTime() + 60 * 60) {
        LogPrintf("CMasternodeBroadcast::SimpleCheckMnRel -- Signature rejected, too far into the future: masternode=%s\n", vin.prevout.ToStringShort());
        nDos = 1;
        return false;
    }

    // empty ping or incorrect sigTime/unknown blockhash
    if(lastPing == CMasternodePing() || !lastPing.SimpleCheckMnRel(nDos)) {
        // one of us is probably forked or smth, just mark it as expired and check the rest of the rules
        nActiveState = MASTERNODE_EXPIRED;
    }

    if(nProtocolVersion < mnpayments.GetMinMasternodePaymentsProto()) {
        LogPrintf("CMasternodeBroadcast::SimpleCheckMnRel -- ignoring outdated Masternode: masternode=%s  nProtocolVersion=%d\n", vin.prevout.ToStringShort(), nProtocolVersion);
        return false;
    }
    ...
    if(pubkeyScript.size() != 25) {
        LogPrintf("CMasternodeBroadcast::SimpleCheckMnRel -- pubKeyCollateralAddress has the wrong size %d instead of 25\n", pubkeyScript.size());
        nDos = 100;
        return false;
    }
    ...
    if(pubkeyScript2.size() != 25) {
        LogPrintf("CMasternodeBroadcast::SimpleCheckMnRel -- pubKeyMasternode has the wrong size %d instead of 25\n", pubkeyScript2.size());
        nDos = 100;
        return false;
    }

    if(!vin.scriptSig.empty()) {
        LogPrintf("CMasternodeBroadcast::SimpleCheckMnRel -- Ignore Not Empty ScriptSig %s\n",vin.ToString());
        nDos = 100;
        return false;
    }
...
bool CMasternodeBroadcast::SimpleCheck(int& nDos)
{
    ...
    if (pubkeyScript.size() != 25) {
        LogPrintf("CMasternodeBroadcast::SimpleCheck -- pubKeyReward has the wrong size %d instead of 25\n", pubkeyScript.size());
        nDos = 100;
        return false;
    }
    ...
    if (pubkeyScript2.size() != 25) {
        LogPrintf("CMasternodeBroadcast::SimpleCheck -- pubKeyMasternode has the wrong size %d instead of 25\n", pubkeyScript2.size());
        nDos = 100;
        return false;
    }
...
// SPORK_14_MNODES_RELEASE_ENABLED active
bool CMasternodeBroadcast::UpdateMnRel(CMasternode* pmn, int& nDos, CConnman& connman)
{
    nDos = 0;
    ...
    if(pmn->sigTime > sigTime) {
        LogPrintf("CMasternodeBroadcast::UpdateMnRel -- Bad sigTime %d (existing broadcast is at %d) for Masternode %s %s\n",
    ...
    if(pmn->IsPoSeBanned()) {
        LogPrintf("CMasternodeBroadcast::UpdateMnRel -- Banned by PoSe, masternode=%s\n", vin.prevout.ToStringShort());
        return false;
    }

    // IsVnAssociatedWithPubkey is validated once in CheckOutpoint, after that they just need to match
    if(pmn->pubKeyCollateralAddress != pubKeyCollateralAddress) {
        LogPrintf("CMasternodeBroadcast::UpdateMnRel -- Got mismatched pubKeyCollateralAddress and vin\n");
        nDos = 33;
        return false;
    }

    if (!CheckSignatureMnRel(nDos)) {
        LogPrintf("CMasternodeBroadcast::UpdateMnRel -- CheckSignatureMnRel() failed, masternode=%s\n", vin.prevout.ToStringShort());
        return false;
    }
    ...
    if(!pmn->IsBroadcastedWithin(MASTERNODE_MIN_MNB_SECONDS) || (fMasterNode && pubKeyMasternode == activeMasternode.pubKeyMasternode)) {
        ...
        LogPrintf("CMasternodeBroadcast::UpdateMnRel -- Got UPDATED Masternode entry: addr=%s\n", addr.ToString());
        if(pmn->UpdateFromNewBroadcast(*this, connman)) {
            pmn->CheckMnRel();
            Relay(connman);
        }
        masternodeSync.BumpAssetLastTime("CMasternodeBroadcast::UpdateMnRel");
    }
...
bool CMasternodeBroadcast::Update(CMasternode* pmn, int& nDos, CConnman& connman)
{
    ...
    // IsVnAssociatedWithPubkey is validated once in CheckOutpoint, after that they just need to match (SPORK_14_MNODES_RELEASE_ENABLED)
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    if(fMasterNode && vin.prevout == activeMasternode.outpoint && pubKeyMasternode == activeMasternode.pubKeyMasternode) {
        LogPrintf("CMasternodeBroadcast::CheckOutpoint -- masternode with the same vin, masternode=%s\n", vin.prevout.ToStringShort());
        return false;
    }

    if (!CheckSignatureMnRel(nDos)) {
        LogPrintf("CMasternodeBroadcast::CheckOutpoint -- CheckSignatureMnRel() failed, masternode=%s\n", vin.prevout.ToStringShort());
        return false;
    }

    {
        ...
        if (err == COLLATERAL_INVALID_AMOUNT) {
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have %d FUTURO, masternode=%s\n", MNODE_REQ_BALANCE, vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 10000 FUTURO tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 1000 FUTURO tx -> 1 confirmation
...
// SPORK_14_MNODES_RELEASE_ENABLED active
bool CMasternodeBroadcast::SignMnRel(const CKey& keyCollateralAddress)
{
    std::string strError;
    ...
    if(!CMessageSigner::SignMessage(strMessage, vchSig, keyCollateralAddress)) {
        LogPrintf("CMasternodeBroadcast::SignMnRel -- SignMessage() failed\n");
        return false;
    }

    if(!CMessageSigner::VerifyMessage(pubKeyCollateralAddress, vchSig, strMessage, strError)) {
        LogPrintf("CMasternodeBroadcast::SignMnRel -- VerifyMessage() failed, error: %s\n", strError);
        return false;
    }
...
// SPORK_14_MNODES_RELEASE_ENABLED active
bool CMasternodeBroadcast::CheckSignatureMnRel(int& nDos)
{
    std::string strMessage;
    std::string strError = "";
    nDos = 0;
    ...
    LogPrint("masternode", "CMasternodeBroadcast::CheckSignatureMnRel -- strMessage: %s  pubKeyCollateralAddress address: %s  sig: %s\n", strMessage, CBitcoinAddress(pubKeyCollateralAddress.GetID()).ToString(), EncodeBase64(&vchSig[0], vchSig.size()));

    if(!CMessageSigner::VerifyMessage(pubKeyCollateralAddress, vchSig, strMessage, strError)){
        LogPrintf("CMasternodeBroadcast::CheckSignatureMnRel -- Got bad Masternode announce signature, error: %s\n", strError);
        nDos = 100;
        return false;
    }
...
// SPORK_14_MNODES_RELEASE_ENABLED active
bool CMasternodePing::SignMnRel(const CKey& keyMasternode, const CPubKey& pubKeyMasternode)
{
    std::string strError;
    ...
    if(!CMessageSigner::SignMessage(strMessage, vchSig, keyMasternode)) {
        LogPrintf("CMasternodePing::SignMnRel -- SignMessage() failed\n");
        return false;
    }

    if(!CMessageSigner::VerifyMessage(pubKeyMasternode, vchSig, strMessage, strError)) {
        LogPrintf("CMasternodePing::SignMnRel -- VerifyMessage() failed, error: %s\n", strError);
        return false;
    }
...
// SPORK_14_MNODES_RELEASE_ENABLED active
bool CMasternodePing::CheckSignatureMnRel(CPubKey& pubKeyMasternode, int &nDos)
{
    std::string strMessage = vin.ToString() + blockHash.ToString() + boost::lexical_cast<std::string>(sigTime);
    std::string strError = "";
    nDos = 0;
    ...
    if(!CMessageSigner::VerifyMessage(pubKeyMasternode, vchSig, strMessage, strError)) {
        LogPrintf("CMasternodePing::CheckSignatureMnRel -- Got bad Masternode ping signature, masternode=%s, error: %s\n", vin.prevout.ToStringShort(), strError);
        nDos = 33;
        return false;
    }
    return true;
...
{
    std::string strMessage = pubKeyMasternode.GetID().ToString() + blockHash.ToString() + boost::lexical_cast<std::string>(sigTime);
    std::string strError = "";
    nDos = 0;
...
// SPORK_14_MNODES_RELEASE_ENABLED active
bool CMasternodePing::SimpleCheckMnRel(int& nDos)
{
    // don't ban by default
    nDos = 0;

    if (sigTime > GetAdjustedTime() + 60 * 60) {
        LogPrintf("CMasternodePing::SimpleCheckMnRel -- Signature rejected, too far into the future, masternode=%s\n", vin.prevout.ToStringShort());
        nDos = 1;
        return false;
    }

    {
        ...
        if (mi == mapBlockIndex.end()) {
            LogPrint("masternode", "CMasternodePing::SimpleCheckMnRel -- Masternode ping is invalid, unknown block hash: masternode=%s blockHash=%s\n", vin.prevout.ToStringShort(), blockHash.ToString());
            // maybe we stuck or forked so we shouldn't ban this node, just fail to accept this ping
            // TODO: or should we also request this block?
            return false;
        }
    }
    LogPrint("masternode", "CMasternodePing::SimpleCheckMnRel -- Masternode ping verified: masternode=%s  blockHash=%s  sigTime=%d\n", vin.prevout.ToStringShort(), blockHash.ToString(), sigTime);
    return true;
}
...
// SPORK_14_MNODES_RELEASE_ENABLED active
bool CMasternodePing::CheckAndUpdateMnRel(CMasternode* pmn, bool fFromNewBroadcast, int& nDos, CConnman& connman)
{
    // don't ban by default
    nDos = 0;

    if (!SimpleCheckMnRel(nDos)) {
        return false;
    }

    if (pmn == NULL) {
        LogPrint("masternode", "CMasternodePing::CheckAndUpdateMnRel -- Couldn't find Masternode entry, masternode=%s\n", vin.prevout.ToStringShort());
        return false;
    }

    if(!fFromNewBroadcast) {
        if (pmn->IsUpdateRequired()) {
            LogPrint("masternode", "CMasternodePing::CheckAndUpdateMnRel -- masternode protocol is outdated, masternode=%s\n", vin.prevout.ToStringShort());
            return false;
        }

        if (pmn->IsNewStartRequired()) {
            LogPrint("masternode", "CMasternodePing::CheckAndUpdateMnRel -- masternode is completely expired, new start is required, masternode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
    }

    {
        ...
        if ((*mi).second && (*mi).second->nHeight < chainActive.Height() - 24) {
            LogPrintf("CMasternodePing::CheckAndUpdateMnRel -- Masternode ping is invalid, block hash is too old: masternode=%s  blockHash=%s\n", vin.prevout.ToStringShort(), blockHash.ToString());
            // nDos = 1;
            return false;
        }
    }

    LogPrint("masternode", "CMasternodePing::CheckAndUpdateMnRel -- New ping: masternode=%s  blockHash=%s  sigTime=%d\n", vin.prevout.ToStringShort(), blockHash.ToString(), sigTime);

    // LogPrintf("mnping - Found corresponding mn for vin: %s\n", vin.prevout.ToStringShort());
    // update only if there is no known ping for this masternode or
    // last ping was more then MASTERNODE_MIN_MNP_SECONDS-60 ago comparing to this one
    if (pmn->IsPingedWithin(MASTERNODE_MIN_MNP_SECONDS - 60, sigTime)) {
        LogPrint("masternode", "CMasternodePing::CheckAndUpdateMnRel -- Masternode ping arrived too early, masternode=%s\n", vin.prevout.ToStringShort());
        //nDos = 1; //disable, this is happening frequently and causing banned peers
        return false;
    }

    if (!CheckSignatureMnRel(pmn->pubKeyMasternode, nDos)) return false;
    ...
    if(!masternodeSync.IsMasternodeListSynced() && !pmn->IsPingedWithin(MASTERNODE_EXPIRATION_SECONDS/2)) {
        ...
        LogPrint("masternode", "CMasternodePing::CheckAndUpdateMnRel -- bumping sync timeout, masternode=%s\n", vin.prevout.ToStringShort());
        masternodeSync.BumpAssetLastTime("CMasternodePing::CheckAndUpdateMnRel");
    }

    // let's store this ping as the last one
    LogPrint("masternode", "CMasternodePing::CheckAndUpdateMnRel -- Masternode ping accepted, masternode=%s\n", vin.prevout.ToStringShort());
    pmn->lastPing = *this;
    ...
    pmn->CheckMnRel(true); // force update, ignoring cache
    if (!pmn->IsEnabled()) return false;

    LogPrint("masternode", "CMasternodePing::CheckAndUpdateMnRel -- Masternode ping acceepted and relayed, masternode=%s\n", vin.prevout.ToStringShort());
    Relay(connman);
