...
DBErrors CWalletDB::LoadWallet(CWallet* pwallet)
{
    ...
    try {
        ...
        // Store initial external keypool size
...
void ThreadFlushWalletDB(const string& strFile)
{
    ...
    RenameThread("futurocoin-wallet");
...
bool AutoBackupWallet (CWallet* wallet, std::string strWalletFile, std::string& strBackupWarning, std::string& strBackupError)
{
    ...
    if(nWalletBackups > 0)
    {
        ...
        if (wallet)
        {
            ...
            if (wallet->IsLocked()) {
                strBackupWarning = _("Wallet is locked, can't replenish keypool! Automatic backups are disabled, please unlock your wallet to replenish keypool.");
                LogPrintf("%s\n", strBackupWarning);
                nWalletBackups = -2;
                return false;
            }
