...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        FuturoCoin Related Inventory Messages
    ...
    */
    ...
    case MSG_MNLIST:
        return masternodesInventoryMap.count(inv.hash);
    }
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        LogPrint("net", "ProcessGetData -- inv = %s\n", inv.ToString());
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!pushed && inv.type == MSG_MNLIST) {
                    if(masternodesInventoryMap.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << masternodesInventoryMap[inv.hash];
                        connman.PushMessage(pfrom, NetMsgType::MNLISTUPDATE, ss);
                        pushed = true;
                    }
                }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    else
    {
        ...
        if (found)
        {
            ...
            masternodeListManager.ProcessMNList(pfrom, strCommand, vRecv, connman);
